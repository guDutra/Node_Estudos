...
class UserController {
    ...
    async create(req, res) {
        try {
            ...
            if (email == undefined || email == '' || email == ' ' || name == undefined || name == '' || name == ' ' || password == undefined || password == '' || password == ' ') {
                res.status(400);
                res.json({ err: "Requisição invalida - Não foi preenchido todos os campos" })
                return;
            }
    ...
    async login(req, res) {
        try {
            ...
            if (user != undefined) {
                ...
                res.status(404);
                res.json({status: false, err: 'Não foi encontrado usuário com este email'})
            }
        } catch (error) {
            console.error(error);
        }
